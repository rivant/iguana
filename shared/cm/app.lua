...
function cm.app.importList(R)
   ...
   return {repository=Config:repoList(), list=L, serverName=xml.parse(iguana.status()).IguanaStatus.ServerName}
end
...
function cm.app.addChannel(R)
   ...
   local Auth = {}
   urlcode.parseQuery(R.body, Auth)
   trace(Auth)
   ...
   local stats, exists
   if Success then      
      stats = Api:listChannels{}.IguanaStatus
   else
      return {success=false}
   end
   
   for i = 1,tonumber(stats.NumberOfChannels:S()) do 
      if stats:child('Channel', i).Name:S() == ChannelName then 
         exists = true
      end
   end
...
function cm.app.currentServer() 
   return xml.parse(iguana.status()).IguanaStatus.ServerName
end

cm.actions = {
   ['config_info'] = cm.app.configInfo,
   ['list-channels'] = cm.app.listChannels,
   ['remote-channels'] = cm.app.remoteChannels,
   ...
   ['current_server'] = cm.app.currentServer
}
